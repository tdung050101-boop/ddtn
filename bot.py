...
async def announce_in_report_channel(message: str) -> None:
    """Gửi thông báo công khai vào kênh báo cáo, không nhắn trong chat voice."""
    try:
        channel = await resolve_report_channel()
        await channel.send(message)
    except discord.HTTPException:
        log.exception("Không gửi được thông báo vào kênh báo cáo")
...
async def start_fixed_shift(shift_day: date, shift: Shift, now: datetime) -> None:
    ...
    await announce_in_report_channel(
...
async def start_manual_command(interaction: discord.Interaction, shift_key: str) -> None:
    ...
    try:
        ...
        report_channel = await resolve_report_channel()
        await report_channel.send(
            f"🟢 **Đã bắt đầu {shift.label} linh động lúc {now_local:%H:%M:%S}.**\n"
            f"Người mở ca: {interaction.user.mention}\n"
        ...
        await interaction.followup.send(
            f"✅ Đã bắt đầu **{shift.label}**. Thông báo đã gửi vào <#{CONFIG.report_channel_id}>.",
            ephemeral=True,
        )
...
async def end_manual_command(interaction: discord.Interaction, shift_key: str) -> None:
    ...
    try:
        ...
        await interaction.followup.send(
            f"✅ Đã kết thúc **{shift.label}**. Kết quả và Excel đã gửi vào "
            f"<#{CONFIG.report_channel_id}>.",
            ephemeral=True,
        )
    except Exception as exc:
...
async def toggle_special_activity_command(
    ...
    if active is not None:
        ...
        try:
            ...
            await interaction.followup.send(
                f"✅ Đã kết thúc **{activity.label}**. Kết quả và Excel đã gửi vào "
                f"<#{CONFIG.report_channel_id}>.",
                ephemeral=True,
            )
        except Exception as exc:
    ...
    try:
        ...
        await report_channel.send(
            ...
            f"Người mở hoạt động: {interaction.user.mention}\n"
        ...
        await interaction.followup.send(
            f"✅ Đã bắt đầu **{activity.label}**. Thông báo đã gửi vào "
            f"<#{CONFIG.report_channel_id}>.",
            ephemeral=True,
        )
    except Exception as exc:
